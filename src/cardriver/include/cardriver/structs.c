...
void writeIMU(imu* p_imu, uint8_t byte, uint16_t index)
{
  switch (index) {
  case 0: // ax MS byte
    p_imu->a_x = ( p_imu->a_x & 0x00ff) | ( ((uint16_t)byte) << 8 );
    break;
  case 1: // ax LS byte
    p_imu->a_x = ( p_imu->a_x & 0xff00) | (uint16_t)byte;
    break;
  case 2:
    p_imu->a_y = ( p_imu->a_y & 0x00ff) | ( ((uint16_t)byte) << 8 );
    break;
  case 3:
    p_imu->a_y = ( p_imu->a_y & 0xff00) | (uint16_t)byte;
    break;
  case 4:
    p_imu->a_z = ( p_imu->a_z & 0x00ff) | ( ((uint16_t)byte) << 8 );
    break;
  case 5:
    p_imu->a_z = ( p_imu->a_z & 0xff00) | (uint16_t)byte;
    break;
  case 6:
    p_imu->g_x = ( p_imu->g_x & 0x00ff) | ( ((uint16_t)byte) << 8 );
    break;
  case 7:
    p_imu->g_x = ( p_imu->g_x & 0xff00) | (uint16_t)byte;
    break;
  case 8:
    p_imu->g_y = ( p_imu->g_y & 0x00ff) | ( ((uint16_t)byte) << 8 );
    break;
  case 9:
    p_imu->g_y = ( p_imu->g_y & 0xff00) | (uint16_t)byte;
    break;
  case 10:
    p_imu->g_z = ( p_imu->g_z & 0x00ff) | ( ((uint16_t)byte) << 8 );
    break;
  case 11:
    p_imu->g_z = ( p_imu->g_z & 0xff00) | (uint16_t)byte;
    break;
  default:
    break;
  }
...
uint8_t readIMU(imu* p_imu, uint16_t index)
{
  ...
  case 0: // ax MS byte
    return p_imu->a_x >> 8;
  case 1: // ax LS byte
    return p_imu->a_x & 0xff;
  case 2:
    return p_imu->a_y >> 8;
  case 3:
    return p_imu->a_y & 0xff;
  case 4:
    return p_imu->a_z >> 8;
  case 5:
    return p_imu->a_z & 0xff;
  case 6:
    return p_imu->g_x >> 8;
  case 7:
    return p_imu->g_x & 0xff;
  case 8:
    return p_imu->g_y >> 8;
  case 9:
    return p_imu->g_y & 0xff;
  case 10:
    return p_imu->g_z >> 8;
  case 11:
    return p_imu->g_z & 0xff;
  default:
    return 0;
  }
}
...
id updateData(uint8_t data, returnT* rvalues){
  ...
  static returnT new_rvalues ={ {0,0},{0},{0,0,0} }; //#### more struct, more initializer? { {0,0} }
  ...
  if(startFlag == true){
    ...
    case 3:
      ...
      case ir_t:
        ...
        break;
      case imu_t:
        new_rvalues.imuSensor.a_x = 0;
        new_rvalues.imuSensor.a_y = 0;
        new_rvalues.imuSensor.a_z = 0;
        new_rvalues.imuSensor.g_x = 0;
        new_rvalues.imuSensor.g_y = 0;
        new_rvalues.imuSensor.g_z = 0;
        break;
      case NAT:
        break;
      }
